...
describe('The case', () => {
    ...
    const N = 16;
    _(N).times(n => {
        it(`First player should send direct message to second player #${n + 1}`, done => {
            let callbackFn = msg => {
                expect(msg).to.deep.equal(`-{direct-${n + 1}`);
            ...
            wsConnection1.send(`-{direct-${n + 1}`);
        });
        it(`Second player should send direct message to first player #${n + 1}`, done => {
            let callbackFn = msg => {
                expect(msg).to.deep.equal(`-{direct-${n + 1}`);
