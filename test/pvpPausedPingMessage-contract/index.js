...
describe('The case', () => {
    ...
    describe('The case', () => {
        ...
        it('Seconds player should disconnect and first player should see a message about it', done => {
            ...
            let callbackFn = () => {
                if(disconnected && firstPlayerMessage){
                    ...
                    expect(firstPlayerMessage.from).to.be.above(0);
        ...
        it('First player should send pause-message ping and get the data about pause', done => {
            let callbackFn = msg => {
                msg = JSON.parse(msg);
                expect(msg).to.have.property('from');
                expect(msg.from).to.be.above(0);
                delete msg.from;
                expect(msg).to.deep.equal({ p: 4, c: 3, paused: 1, turn: 0 });
        ...
        it('Second player should connect again and first player should see a message about it', done => {
            ...
            let callbackFn = () => {
                if(openIsOk && secondPlayerConnectedMessage && firstPlayerMessage){
                    ...
                    expect(secondPlayerConnectedMessage.from).to.be.equal(firstPlayerMessage.from);
                    delete secondPlayerConnectedMessage.from;
                    expect(secondPlayerConnectedMessage).to.have.property('to');
                    expect(secondPlayerConnectedMessage.to).to.be.equal(firstPlayerMessage.to);
        ...
        it('First player should send pause-message ping and get the data about unpause', done => {
            let callbackFn = msg => {
                msg = JSON.parse(msg);
                expect(msg).to.have.property('from');
                expect(msg.from).to.be.above(0);
                expect(msg).to.have.property('to');
                expect(msg.to).to.be.above(msg.from);
                delete msg.from;
                delete msg.to;
                expect(msg).to.deep.equal({ p: 4, c: 3, paused: 0, turn: 0 });
