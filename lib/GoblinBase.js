...
var WorkloadControlConfig = require('./config/WorkloadControlConfig.js'),
    ...
    ChatsConfig = require('./config/ChatsConfig.js');
...
class GoblinBase{
    ...
    configChats(opts={}){
        this.chatsConfig = new ChatsConfig(opts);
        return this;
    }
    ...
    start(port, host, apiPrefix, callback=()=>{}){
        function checkIntegrity(){
            ...
            if(this.chatsConfig && !this.redisConfig.mainWebApp.chat){
                configsToGo.push('If chats included - chat redis client is the must');
            }
    ...
    _clearDatabases(callback){
        require('async').parallel([
            ...
            cb => require('./persistenceSubsystem/dao/chatMessage.js').deleteMany({}, cb),
            cb => require('./persistenceSubsystem/dao/chatGroup.js').deleteMany({}, cb),
