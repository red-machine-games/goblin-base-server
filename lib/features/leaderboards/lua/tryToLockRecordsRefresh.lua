...
local refresh = redis.call('hmget', 'refreshl', 'locked', 'lock_key', 'now', 'from', 'to')

if refresh[1] ~= false then
    if refresh[1] == '1' then
        if tonumber(refresh[3]) + PACKAGE_TIMEOUT <= NOW then
            local new_lock_key = tonumber(refresh[2]) + 1
            redis.call('hmset', 'refreshl', 'lock_key', tostring(new_lock_key), 'now', tostring(NOW))
    ...
    elseif tonumber(refresh[4]) >= tonumber(refresh[5]) then
        if tonumber(refresh[3]) + ALL_REFRESH_TIMEOUT <= NOW then
            local new_lock_key = tonumber(refresh[2]) + 1
            redis.call('hmset', 'refreshl', 'locked', '1', 'lock_key',
        ...
        if tonumber(refresh[3]) + PACKAGE_TIMEOUT <= NOW then
            local new_lock_key = tonumber(refresh[2]) + 1
            redis.call('hmset', 'refreshl', 'locked', '1', 'lock_key', tostring(new_lock_key), 'now', tostring(NOW))
            return { 1, tonumber(refresh[4]), tonumber(refresh[5]), new_lock_key }
        end
    end
else
    redis.call('hmset', 'refreshl', 'locked', '1', 'lock_key', '1', 'now', tostring(NOW), 'from', '0', 'to', '-1')
    return { 1, 1 }
end
