...
const SUBSCRIPTION_LIFETIME_MS = 15000,
    LONG_POLLING_COLD_RESPONSE_AFTER_MS = 5000,
    CHANNEL_NAME = 'grchat',
    MAX_LISTING_LIMIT_VALUE = 50,
    SUBSCRIPTIONS_REFRESH_BATCH_SIZE = 100,
    SUBSCRIPTIONS_REFRESH_RELOADING_TIME_MS = 1000,
    MAX_INBOX_PER_BUCKET = 50,
    MESSAGE_MAX_SIZE = 256,
    LAZY_NO_REFRESH_CHATS_MS = 500;
...
function init(){
    ...
    opClients.getChatListenerClient().getRedis().subscribe(CHANNEL_NAME);
...
function tryToRefreshChats(now, callback){
    (async () => {
        if(now - lazyNoRefreshChatsTs > LAZY_NO_REFRESH_CHATS_MS){
            lazyNoRefreshChatsTs = now;
            try{
                await opClients.getChatClient().tryToRefreshChatSubscriptionsAsync(
                    [now, SUBSCRIPTIONS_REFRESH_RELOADING_TIME_MS, SUBSCRIPTIONS_REFRESH_BATCH_SIZE, SUBSCRIPTION_LIFETIME_MS]
                );
            } catch(err){
                log.error('OP Error', { code: 9999, err: { code: err.code, command: err.command, message: err.message } });
                throw new ErrorResponse(9999, 'OP Error');
            }
...
function opHandleMessage(channel, message){
    ...
    async function workoutParticularLpConnection(inGroup, humanId, lpConnection){
        ...
        async function tryToFetchMessages(){
            ...
            try{
                response = await opClients.getChatClient()
                    .tryToFetchGroupChatMessagesAsync([humanId, inGroup, now, SUBSCRIPTION_LIFETIME_MS]);
            } catch(err){
                log.error('OP Error', { code: 9999, err: { code: err.code, command: err.command, message: err.message } });
                return lpConnection.close(500, new ErrorResponse(9999, 'OP Error'));
            }
        ...
        async function tryToResubscribe(){
            ...
            try{
                await opClients.getChatClient().tryToRefreshChatSubscriptionsAsync(
                    [resubscriptionNow, SUBSCRIPTIONS_REFRESH_RELOADING_TIME_MS, SUBSCRIPTIONS_REFRESH_BATCH_SIZE, SUBSCRIPTION_LIFETIME_MS]
                );
            } catch(err){
                log.error('OP Error', { code: 9999, err: { code: err.code, command: err.command, message: err.message } });
                return lpConnection.close(500, new ErrorResponse(9999, 'OP Error'));
    ...
    if(channel === CHANNEL_NAME && longPollingConnections){
...
function postMessage(sessionObject, theMessage, inGroup, noPersist, callback){
    ...
    function checkSession(){
        if(sessionObject.pcrd && sessionObject.pid && sessionObject.hid){
            ...
            justUnlockSession(400, new ErrorResponse(9999, 'You do not have a profile or should get one'));
        }
    }
    function validateTheMessage(){
        if(!noPersist && !messagesStoring.currentlyIsOkay()){
            return justUnlockSession(503, new ErrorResponse(9999, 'Chat persistence currently unavailable'));
        }
        if(_.isEmpty(theMessage) || Buffer.byteLength(theMessage, 'utf8') > MESSAGE_MAX_SIZE){
            return justUnlockSession(400, new ErrorResponse(9999, 'Message value has invalid size'));
        }
        ...
        postMessageImplementation(now, sessionObject.hid, theMessage, inGroup, noPersist)
            ...
            .catch(err => {
                if(err instanceof WebResponseWithCode){
                    ...
                    log.error(err);
                    justUnlockSession(500, new ErrorResponse(9999, 'Some server-side error (logged)'));
                }
            });
    }
...
async function postMessageImplementation(now, humanId, theMessage, inGroup, noPersist){
    ...
    async function tryToRefreshSubscriptions(){
        try{
            ...
            log.error('OP Error', { code: 9999, err: { code: err.code, command: err.command, message: err.message } });
            throw new WebResponseWithCode(500, new ErrorResponse(9999, 'OP Error'));
        }
    ...
    async function pushMessage(){
        try{
            let response = await opClients.getChatClient().pushNewChatMessageAsync([jsonpack.pack({
                ...
                cat: now
            }), inGroup, newMessageSequenceNum, MAX_INBOX_PER_BUCKET]);
...
function listMessages(sessionObject, skip, limit, fromGroup, fromCat, callback){
    ...
    function validateInput(){
        if(!_.isEmpty(goblinBase.chatsConfig.chatGroupWhitelist) && !goblinBase.chatsConfig.chatGroupWhitelist.includes(fromGroup)){
            return justUnlockSession(400, new ErrorResponse(9999, 'Trying to list messages of forbidden group'));
        }
        if(isNaN(skip) || skip < 0 || skip > Number.MAX_SAFE_INTEGER){
            return justUnlockSession(400, new ErrorResponse(9999, 'Invalid "skip" argument'));
        }
        if(isNaN(limit) || limit < 0 || limit > MAX_LISTING_LIMIT_VALUE){
            return justUnlockSession(400, new ErrorResponse(9999, 'Invalid "limit" argument'));
        }
        if(isNaN(fromCat) || fromCat < 0 || fromCat > Number.MAX_SAFE_INTEGER){
            return justUnlockSession(400, new ErrorResponse(9999, 'Invalid "fromCat" argument'));
        }
        return doList();
    }
    function doList(){
...
function fetchMessages(sessionObject, fromGroup, theRequest, theResponse, callback){
    function checkSession(){
        if(sessionObject.pcrd && sessionObject.pid && sessionObject.hid){
            ...
            justRespond(400, new ErrorResponse(9999, 'You do not have a profile or should get one'));
        }
    }
    function validateInput(){
        if(!_.isEmpty(goblinBase.chatsConfig.chatGroupWhitelist) && !goblinBase.chatsConfig.chatGroupWhitelist.includes(fromGroup)){
            return justRespond(400, new ErrorResponse(9999, 'Trying to list messages of forbidden group'));
        }
        return doFetch();
    }
    function doFetch(){
        fetchMessagesImplementation(sessionObject.hid, fromGroup, new LongPollingWrapper(theRequest, theResponse))
            ...
            .catch(err => {
                if(err instanceof WebResponseWithCode){
                    ...
                    log.error(err);
                    justRespond(500, new ErrorResponse(9999, 'Some server-side error (logged)'));
                }
            });
    }
...
async function fetchMessagesImplementation(humanId, fromGroup, lpConnection){
    ...
    async function tryToSubscribe(){
        try{
            let response = await opClients.getChatClient()
                .tryToSubscribeGroupChatMessagesAsync([humanId, fromGroup, now, SUBSCRIPTION_LIFETIME_MS]);
    ...
    async function tryToFetchTheseThings(){
        ...
        try{
            response = await opClients.getChatClient()
                .tryToFetchGroupChatMessagesAsync([humanId, fromGroup, now, SUBSCRIPTION_LIFETIME_MS]);
        } catch(err){
            log.error('OP Error', { code: 9999, err: { code: err.code, command: err.command, message: err.message } });
            throw new WebResponseWithCode(500, new ErrorResponse(9999, 'OP Error'));
        }
        ...
        if(response === 0){
            ...
            return output = new WebResponseWithCode(400, new ErrorResponse(9999, 'Unexpected no subscription'));
        }
    ...
    function addNewLongPollingConnection(){
        if(longPollingConnections){
            ...
            if(checkExistentLp){
                checkExistentLp.close(400, new ErrorResponse(9999, 'Closed because of concurrent connection'));
            }

            lpConnection.setColdResponse(200, { mess: [], subscribed: subscribed ? now : -1 }, LONG_POLLING_COLD_RESPONSE_AFTER_MS);
            lpConnection.payload('subscribed', subscribed ? now : -1);
            longPollingConnections.add(lpConnection, where);
        } else {
            return output = new WebResponseWithCode(400, new ErrorResponse(9999, 'No more long polling connections allowed'));
        }
    }
