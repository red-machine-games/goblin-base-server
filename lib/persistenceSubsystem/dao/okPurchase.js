...
const COLLECTION_NAME = 'okpurchases';
        ...
        if(autoIndex){
            let callbackFn = (err, results) => {
                if(err){
                    ...
                    let err = new ErrorResponse(863, 'Some database indexes were not created');
                    log.fatal(err);
                    callback(err);
                }
            };
            ...
            async.series([
                cb => theCollection.createIndex({ txid: 1 }, { unique: true }, cb),
