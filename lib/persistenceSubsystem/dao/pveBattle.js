...
const COLLECTION_NAME = 'pvebattles',
        ...
        if(autoIndex){
            let callbackFn = (err, results) => {
                if(err){
                    ...
                    let err = new ErrorResponse(869, 'Some database indexes were not created');
                    log.fatal(err);
                    callback(err);
                }
            };
            ...
            async.series([
                ...
                cb => theCollection.createIndex({ pid: 1, cat: 1 }, { unique: true }, cb)
            ], callbackFn);
        } else {
            blankSearch();
        }
    ...
    function rebuildIndexes(){
        let callbackFn = (err, results) => {
            if(err){
                ...
                let err = new ErrorResponse(870, 'Some database indexes were not created');
                log.fatal(err);
                callback(err);
            }
        };
